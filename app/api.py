    ...
    def post(self):
       ...
       # Returns if host is already used in list.
        if host in records.keys():
            return jsonify(status_code='200', success=False,
                           msg="The hostname %s is already registered to the ip address %s." % (host, records[host]))

            # writes to /etc/os_hosts.txt if the hostname is not found in file.
        else:
            hostwritetofile(self, host, ip, records)
            # restart_service('dnsmasq')
            return jsonify(status_code="200", success=True,
                           msg="The hostname %s has been registered to the ip address %s." % (host, ip), data=args)
        ...
        # if hostname is blank it will query the ipaskey dict and delete record using IP address. Will error if IP address is not found in ipaskey dict.
            ...
            try:
                ...
                with open('/etc/hosts.txt', 'r+b') as f:
                    ...
                    f.truncate()
                    return jsonify(msg="The record for %s registered to %s has  been removed." % (ip, hostname),
...
class UserAPI(Resource):
    ...
    def post(self):
        ...
        if username is None or password is None:
            return jsonify(msg="Please enter a valid username or password.", success=False,
                           status_code='200')  # missing arguments
...
def hostaskey(self, records):
    with open('/etc/os_hosts.txt', 'r+b') as f:
        for line in f:
            k, v = line.split("  ")
            records[v.strip()] = k.strip()

def hostwritetofile(self, host, ip, records):
        with open('/etc/hosts.txt', 'r+b') as f:
            records[host] = ip
            f.writelines('{}  {}\n'.format(v, k) for k, v in records.items())
            f.truncate()